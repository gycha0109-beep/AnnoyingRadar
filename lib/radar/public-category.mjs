...
export function publicCategoryLabel(problem) {
  const category = normalize(problem?.category);
  const text = publicText(problem);

  for (const label of PUBLIC_CATEGORY_CHIPS) {
    const rule = RULES[label];
    if (rule.categoryPrefixes.some((prefix) => category.startsWith(prefix))) return label;
    if (rule.searchTerms.some((term) => text.includes(normalize(term)))) return label;
  }
