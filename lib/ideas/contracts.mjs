...
export function normalizeIdeaStatusRequest(value, currentStatus) {
  assertPlainObject(value, "Idea status request");
  const keys = Object.keys(value);
  if (keys.length !== 1 || keys[0] !== "status") {
    throw new TypeError("Idea status request must contain only status");
  }

  const targetStatus = normalizeIdeaStatus(value.status);
