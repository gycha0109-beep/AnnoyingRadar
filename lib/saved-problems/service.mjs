...
export async function loadSavedProblemComparison(serviceClient, userId, candidateIds) {
  if (!candidateIds.length) return [];

  const { data: savedProblems, error: savedError } = await serviceClient
    .from("ar_saved_problem_cards")
    .select(SAVED_PROBLEM_SELECT)
    .eq("user_id", userId)
  ...
  const attached = await attachProblemCards(serviceClient, userId, savedProblems);
