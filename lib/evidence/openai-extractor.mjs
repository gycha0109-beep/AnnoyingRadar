...
export async function extractPainEvidence({
  ...
  try {
    response = await fetchImpl("https://api.openai.com/v1/responses", {
      ...
      body: JSON.stringify(
        buildOpenAIRequest({ rawText: normalizedRawText, sourceLanguage, model }),
      ),
...
export function buildOpenAIRequest({ rawText, sourceLanguage, model }) {
  return {
    model,
    store: false,
    instructions: [
      ...
      "Return only pains that are directly supported by the source text.",
            ...
            evidences: {
              ...
              items: {
                ...
                properties: {
                  ...
                  sentiment_level: {
                    type: "string",
                    enum: ["negative", "mixed", "neutral", "unknown"],
                  },
                  intensity_level: {
                    type: "string",
...
export function normalizeExtractedEvidence(value, rawText) {
  ...
  return value.evidences.map((item, index) => {
    ...
    if (!String(rawText).includes(originalText)) {
