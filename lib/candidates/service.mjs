...
export async function loadCandidateReview(
  ...
  if (!includeDiscarded) query = query.neq("status", "discarded");
...
export async function loadCandidateDetail(serviceClient, candidateId, userId) {
  ...
  return {
    raw_input: rawInput,
    candidate: candidates.find((item) => item.id === candidateId) ?? { ...candidate, evidences: [] },
    sibling_candidates: candidates.filter((item) => item.id !== candidateId),
  };
}
