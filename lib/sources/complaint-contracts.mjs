import { classifySourceAdmission } from "./source-admission.mjs";
...
export const COMPLAINT_REASON_CODES = Object.freeze([
  ...
  "source_title_information_or_guide",
  "source_title_commercial_or_seo",
  "source_title_product_or_promotion",
  "source_title_borrowed_pain_leadgen",
  "source_title_positive_review",
  "source_title_mixed_information_and_experience",
  "source_title_mixed_commercial_and_experience",
  "source_title_mixed_positive_and_complaint",
  "source_title_explicit_complaint_requires_context",
  "source_title_explicit_first_hand_complaint",
  "source_title_actual_experience_complaint_central",
  "source_title_concrete_cost_loss_requires_context",
  "source_title_long_wait_harm_requires_context",
  "source_title_long_wait_harm_complaint_central",
  "source_title_systemic_service_access_harm_requires_context",
  "source_title_systemic_service_access_harm_complaint_central",
  "source_title_warning_problem_requires_context",
  "source_title_warning_problem_complaint_central",
  "source_title_truncated_complaint_ambiguous",
  "source_title_truncated_topic_without_event",
  "source_title_truncated_no_complaint_signal",
  "source_title_complaint_experience_ambiguous",
  "source_title_topic_without_event",
  "source_title_no_complaint_signal",
  "source_snippet_first_hand_complaint_requires_context",
  "source_snippet_strong_complaint_requires_context",
  "source_snippet_first_hand_friction_requires_context",
  "source_snippet_high_confidence_direct_harm",
  "source_snippet_incidental_complaint_only",
  "source_snippet_incidental_complaint_requires_context",
  "source_snippet_pain_hook_or_promotion",
  "source_snippet_information_only",
  "source_snippet_positive_context",
  "other",
]);
...
const GENERIC_NEGATIVE = /^(?:오늘\s*)?(?:진짜\s*)?(?:짜증|짜증나|짜증난다|빡침|빡친다|별로|최악|개망|망했네|싫다|노답|ugh|annoying|this sucks|hate it|terrible)[.!?ㅋㅎ\s]*$/i;
...
export function runDeterministicComplaintPrefilter(signal) {
  ...
  if (!rawText || URL_ONLY.test(rawText)) {
    return { decision: "reject", reason_codes: ["link_only_or_no_claim"] };
  }
  ...
  if (admission.decision === "reject") {
    return {
      decision: "reject",
      reason_codes: admission.reason_codes.map((code) => `source_${code}`),
    };
  }
  if (admission.decision === "review") {
    reasons.push(...admission.reason_codes.map((code) => `source_${code}`));
  }

  if (signal?.is_quote_post === true) reasons.push("repost_or_copy");
  ...
  if (reasons.length > 0) return { decision: "review", reason_codes: [...new Set(reasons)] };
  return { decision: "continue", reason_codes: [] };
}

export function deriveComplaintDecision({ complaint_relevant, first_hand_experience, concrete_friction }) {
  for (const [field, value] of Object.entries({ complaint_relevant, first_hand_experience, concrete_friction })) {
  ...
  if (complaint_relevant === "yes" && first_hand_experience === "yes" && concrete_friction === "yes") return "pass";
  if (complaint_relevant === "no" || first_hand_experience === "no" || concrete_friction === "no") return "reject";
  return "review";
}
...
export function normalizeGoldAnnotationInput(value, rawText = null) {
  ...
  if (unknown) throw new ComplaintContractError("invalid_gold_annotation", `Unsupported Gold annotation field: ${unknown}`);
  ...
  if (complaintRelevant === "yes") {
    ...
    if (!coreEvidence) {
      throw new ComplaintContractError("invalid_gold_positive", "complaint_relevant=yes requires core_evidence");
    }
  }
...
function triState(value, fieldName) {
  if (!TRI_STATE.has(value)) {
    throw new ComplaintContractError("invalid_gold_annotation", `${fieldName} must be yes, no, or uncertain`);
  }
  return value;
}
