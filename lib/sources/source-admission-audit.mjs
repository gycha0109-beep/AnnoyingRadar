...
const INDEPENDENT_RISK_RULES = Object.freeze([
  {
    ...
    pattern: /(?:때문에|탓에).{0,48}(?:못\s*(?:타|쓰|이용|받|가|들어|신청|예약|결제|접근|참여)|이용\s*(?:불가|못|어려)|접근\s*(?:불가|못|어려)|사용\s*(?:불가|못|어려))/i,
  },
  {
  ...
  },
  {
    ...
    pattern: /(?:(?:\d[\d,]*(?:\.\d+)?\s*(?:만원|원)|수리비|위약금|추가\s*비용|수수료).{0,44}(?:부담|비싸|과다|폭탄|손해|포기|교체|재구매))|(?:(?:부담|비싸|과다|폭탄|손해).{0,44}(?:\d[\d,]*(?:\.\d+)?\s*(?:만원|원)|수리비|위약금|추가\s*비용|수수료)))/i,
  },
  {
...
export function buildSourceAdmissionIndependentAudit(signals, {
  ...
  return {
    ...
    boundary_set: boundaryRows.map((row) => toAuditItem(row.signal, { set: "boundary" })),
    reject_risk_set: rejectRiskRows.map((row) => toAuditItem(row.signal, {
      set: "reject_risk",
      auditRisk: row.auditRisk,
    })),
    reject_random_set: rejectRandomRows.map((row) => toAuditItem(row.signal, { set: "reject_random" })),
  };
}
...
function toAuditItem(signal, { set, auditRisk = null }) {
  return {
    id: signal.id,
    set,
