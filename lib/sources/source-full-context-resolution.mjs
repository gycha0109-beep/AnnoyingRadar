...
export function resolveFullContextSemantic(semantic) {
  ...
  const candidate = normalized.problem_claim === "yes"
    ...
    && ["organic", "informational"].includes(normalized.content_kind);
  if (candidate) {
...
export async function resolveSourceAdmissionWithFullContext(signal, {
  ...
  const fullContext = await fetchContext(signal, { fetchImpl });
  if (fullContext?.status !== "resolved" || !fullContext?.content_text) {
    return {
      ...
      semantic: null,
      reason_codes: [fullContext?.error_code ?? "full_context_unavailable"],
    };
  }
  ...
  try {
    ...
    return {
      ...
      semantic: null,
      reason_codes: [typeof error?.code === "string" ? error.code : "full_context_judge_failed"],
    };
  }
