...
export const SOURCE_ADMISSION_POLICY_REVISION = "source-admission-v0.8-pain-ownership-v0.1";
export const PAIN_OWNERSHIP_VERSION = "pain-ownership-v0.1";
...
const PROFESSIONAL_LEAD_GEN_AUTHOR = /(?:법무법인|법률사무소|법률상담|변호사|법무사|행정사|노무사|손해사정|고소의달인)/i;
const BORROWED_PAIN_CASE_TITLE = /(?:(?:사기|사칭).*?(?:피해|수법|사례|구조|주의|대응)|피해.*?(?:수법|사례|구조|주의|대응|포인트)|팀미션.*?피해)/i;
const BORROWED_PAIN_CASE_SNIPPET = /(?:사기\s*업체명|사칭\s*사기\s*수법|대부분\s*피해자|피해자(?:는|가|에게|들의?)|피해는\s*주로|(?:사례|경우)가\s*많|선입금\s*요구|팀미션\s*참여|유명\s*쇼핑몰.*?사칭|정상적인\s*구매대행.*?가장)/i;
...
export function classifyPainOwnership(signal) {
  const title = extractSourceTitle(signal);
  const snippet = extractSourceSnippet(signal);
  const author = normalizeSourceText(signal?.author_handle);
  const professionalLeadGen = PROFESSIONAL_LEAD_GEN_AUTHOR.test(author);
  const packagedCase = BORROWED_PAIN_CASE_TITLE.test(title) || BORROWED_PAIN_CASE_SNIPPET.test(snippet);
  const explicitFirstHand = FIRST_HAND_SNIPPET.test(snippet) || /(?:내가|제가|나는|저는)/i.test(title);

  if (professionalLeadGen && packagedCase) {
    return {
      version: PAIN_OWNERSHIP_VERSION,
      ownership: "borrowed_leadgen",
      author,
      professional_leadgen: true,
      explicit_first_hand: explicitFirstHand,
    };
  }
  if (explicitFirstHand) {
    return {
      version: PAIN_OWNERSHIP_VERSION,
      ownership: "owned",
      author,
      professional_leadgen: professionalLeadGen,
      explicit_first_hand: true,
    };
  }
  if (WARNING_REPORT_TITLE.test(title) || SYSTEMIC_SERVICE_ACCESS_HARM_TITLE.test(title)) {
    return {
      version: PAIN_OWNERSHIP_VERSION,
      ownership: "reported",
      author,
      professional_leadgen: professionalLeadGen,
      explicit_first_hand: false,
    };
  }
  return {
    version: PAIN_OWNERSHIP_VERSION,
...
export function classifySourceIntent(signal) {
  ...
  const painOwnership = classifyPainOwnership(signal);

  let intent = "unknown";
  if (painOwnership.ownership === "borrowed_leadgen") intent = "lead_gen";
  else if (painHookPitch || promotionalTitle) intent = "promotion";
  ...
  if (painOwnership.ownership === "borrowed_leadgen") painRole = "borrowed";
  else if (painHookPitch) painRole = "hook";
  ...
  return {
    intent,
    pain_role: painRole,
    pain_ownership: painOwnership.ownership,
    provenance_version: painOwnership.version,
  };
}
...
export function classifySourceAdmission(signal) {
  ...
  if (painOwnership.ownership === "borrowed_leadgen") {
    return admission({
      decision: "reject",
      title,
      snippet,
      reasonCodes: ["title_borrowed_pain_leadgen"],
      requiresFullContext: false,
    });
  }
