...
const EXPLICIT_COMPLAINT_TITLE = /(?:환불\s*(?:안\s*됨|안됨|불가|거절|거부|못\s*받|못받|지연|미처리)|취소\s*(?:당함|당했|거절|안\s*됨|안됨|일방)|일방적\s*취소|(?:로그인|결제|예약|예약조회|주문|배달|호출|인증|이체)\s*(?:오류|실패|안\s*됨|안됨)|(?:오류|실패|지연|누락)\s*때문에|답(?:변)?\s*(?:없음|없다|안\s*옴|안옴)|응답\s*(?:없음|없다)|기다리다\s*(?:지침|지쳐|열받)|(?:진짜\s*|완전\s*|존나\s*)?(?:최악|비추(?:천)?)|먹튀|바가지)/i;
...
export function classifySourceAdmission(signal) {
  ...
  // Source framing outranks retrieval text. Legal/how-to/SEO titles that merely carry
  // complaint vocabulary are not complaint sources. If such a title also contains a
  // genuine experience frame, preserve it for human/full-context review rather than
  // silently hard-rejecting it.
  ...
  // Otherwise it stays REVIEW so legal/SEO phrasing cannot become a candidate merely
  // because it contains "환불 불가" or another query-shaped event phrase.
