...
export function classifySourceAdmission(signal) {
  ...
  // Retrieval snippets can demote before any first-hand preservation. This prevents
  // a stray first-person phrase elsewhere in an everyday post from rescuing an
  // incidental parenthetical query hit such as #10.
  ...
  // A non-incidental first-hand complaint-looking snippet may request context, but
  // it still never creates a candidate.
  if (firstHandSnippet) {
    return admission({
      decision: "review",
      title,
