export const SOURCE_ADMISSION_VERSION = "source-admission-v0.6";
...
export function classifySourceAdmission(signal) {
  ...
  // Title authority is decisive for obvious information/SEO framing. Conditional
  // guide grammar such as "거부당했을 때" is not first-hand narrative evidence.
  ...
  // v0.6 narrow false-negative recovery. These title forms carry concrete harm,
  // but they still require full-source inspection before admission as a candidate.
  if (concreteCostLoss || longWaitHarm) {
    return admission({
      decision: "review",
      title,
      snippet,
      reasonCodes: [concreteCostLoss ? "title_concrete_cost_loss_requires_context" : "title_long_wait_harm_requires_context"],
      requiresFullContext: true,
    });
  }
  ...
  // Retrieval snippets can demote, but they cannot establish source centrality.
  // This prevents a daily post, resale listing, or opaque title from entering REVIEW
  // merely because NAVER selected a first-hand complaint fragment into description.
