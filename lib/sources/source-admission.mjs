...
export function classifySourceAdmission(signal) {
  ...
  // Source framing outranks retrieval snippets. Information/guide titles are rejected
  // even when the search description happens to contain first-hand pain language.
  if (informational) {
    return admission({
      decision: "reject",
      title,
  ...
  // Commercial/SEO and positive-review framing are not complaint-central. If an
  // explicit complaint event is mixed into the title, preserve it for review rather
  // than promoting it automatically.
  ...
  // Candidate is intentionally narrow. Generic nouns such as 사기/피해/분실/도용
  // are only topics; they do not prove that the source itself is a complaint.
  if (explicitComplaint) {
    return admission({
      decision: "candidate",
      title,
  ...
  // Snippet has negative authority only. It may prove retrieval contamination but can
  // never promote a source to candidate.
  ...
  // Truncation by itself is not enough to consume full-context review capacity.
  // Only a truncated title that still carries a complaint topic remains ambiguous.
  if (truncated) {
    if (complaintTopic) {
      return admission({
        decision: "review",
        title,
        snippet,
        reasonCodes: ["title_truncated_complaint_topic"],
        requiresFullContext: true,
      });
    }
    return admission({
      ...
      title,
      snippet,
      reasonCodes: ["title_truncated_no_complaint_signal"],
      requiresFullContext: false,
    });
  }

  // Complaint vocabulary without an explicit event is ambiguous, not a candidate.
  if (complaintTopic) {
    return admission({
      decision: "review",
      title,
      snippet,
      reasonCodes: ["title_complaint_topic_only"],
      requiresFullContext: true,
    });
  }

  // A neutral title can reach REVIEW only when the snippet has multiple independent
  // complaint markers. This still never promotes to candidate and exists for cases
  // such as a genuinely complaint-heavy daily post with an opaque title.
  if (hasCentralComplaintLanguage(snippet)) {
    return admission({
      decision: "review",
      title,
      snippet,
