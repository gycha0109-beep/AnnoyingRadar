...
const STRONG_GUIDE_TITLE = /(?:전화번호|상담시간|총정리|환불\s*규정|해결\s*(?:방법|가이드)|체크리스트|신청\s*(?:방법|가이드)|신고\s*(?:방법|가이드)|하는\s*법|정리(?:해|했|합니다|하기|$)|가이드(?:\s|$)|FAQ|Q&A|법적\s*효력|청약철회|환급금\s*계산|위약금\s*\d+%|요금제\s*비교|아끼는\s*법|예방법)/i;
...
const FIRST_HAND_OR_NARRATIVE = /(?:저는|제가|저도|나는|내가|나도|실제\s*경험|썰|겪(?:은|어|었다|었)|당했(?:다|어요|습니다|음|네요|는데)|했는데|하다가|해보니|가보니|문의했|전화했|기다렸|고생|열받|빡침|결국|처음임)/i;
const STRONG_FRICTION = /(?:환불\s*(?:안\s*됨|안됨|거절|거부|못\s*받|못받|지연|미처리)|취소\s*(?:당함|당했|거절|안\s*됨|안됨|일방)|일방적\s*취소|(?:로그인|결제|예약|예약조회|주문|배달|호출|인증|이체)\s*(?:오류|실패|안\s*됨|안됨)|(?:오류|실패|지연|누락)\s*때문에|답(?:변)?\s*(?:없음|없다|안\s*옴|안옴)|응답\s*(?:없음|없다)|연락\s*(?:두절|안\s*됨|안됨)|먹튀|바가지|헛걸음|미발송|강제집행|내용증명|민원\s*(?:신고|접수))/i;
