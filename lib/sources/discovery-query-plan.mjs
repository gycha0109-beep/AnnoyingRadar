...
export function scoreDiscoveryRunMetrics(metrics) {
  ...
  if (completedRuns < 1 || fetched < 1) {
    return {
      score: 0.55,
      exploration: true,
      rates: { reject_rate: 0, new_signal_rate: 0, duplicate_rate: 0, candidate_rate: 0 },
    };
  }

  const rejectRate = safeRate(rejected, fetched);
  const newSignalRate = safeRate(inserted, Math.max(accepted, 1));
  const duplicateRate = safeRate(duplicates, Math.max(accepted, 1));
  const candidateRate = safeRate(candidates, Math.max(accepted, 1));
  const score = clamp(
    (0.5 * candidateRate)
      + (0.3 * newSignalRate)
      + (0.2 * (1 - rejectRate))
...
export function selectDiscoveryRequestBudget(plan, historicalMetrics = [], { maxRequests = 24 } = {}) {
  ...
  const exploitation = ranked.filter((item) => !item.yield.exploration);
