...
export const DISCOVERY_QUERY_ALLOCATION_VERSION = "source-discovery-allocation-v0.4";
...
function resolveScoringMetrics(metrics) {
  const exactRuns = Number(metrics?.new_telemetry_runs ?? 0);
  if (exactRuns > 0) {
    return {
      scope: "new_source_exact",
      completedRuns: exactRuns,
      fetched: Number(metrics?.new_telemetry_fetched_count ?? 0),
      accepted: Number(metrics?.new_telemetry_continue_count ?? 0),
      rejected: Number(metrics?.new_telemetry_discovery_reject_count ?? 0),
      inserted: Number(metrics?.new_telemetry_inserted_count ?? 0),
      duplicates: Number(metrics?.new_telemetry_duplicate_count ?? 0),
      candidates: Number(metrics?.new_admission_candidate_count ?? 0),
      reviews: Number(metrics?.new_admission_review_count ?? 0),
      admissionRejects: Number(metrics?.new_admission_reject_count ?? 0),
      admissionDenominator: Number(metrics?.new_telemetry_inserted_count ?? 0),
    };
  }

  return {
    scope: "legacy_run_level",
    completedRuns: Number(metrics?.completed_runs ?? 1),
    fetched: Number(metrics?.fetched_count ?? 0),
    accepted: Number(metrics?.discovery_continue_count ?? 0),
    rejected: Number(metrics?.discovery_reject_count ?? 0),
    inserted: Number(metrics?.inserted_count ?? 0),
    duplicates: Number(metrics?.duplicate_count ?? 0),
    candidates: Number(metrics?.admission_candidate_count ?? 0),
    reviews: Number(metrics?.admission_review_count ?? 0),
    admissionRejects: Number(metrics?.admission_reject_count ?? 0),
    admissionDenominator: Number(metrics?.discovery_continue_count ?? 0),
  };
}

export function scoreDiscoveryRunMetrics(metrics) {
  const scoring = resolveScoringMetrics(metrics);
  const {
    scope,
    completedRuns,
    fetched,
  ...
  if (scope === "new_source_exact" && inserted < 1) {
    return {
      score: 0,
      exploration: false,
      telemetry_scope: scope,
  ...
  const candidateRate = safeRate(candidates, admissionDenominator);
  const reviewRate = safeRate(reviews, admissionDenominator);
  const admissionRejectRate = safeRate(admissionRejects, admissionDenominator);
  const score = clamp(
    (0.4 * candidateRate)
      + (0.1 * reviewRate)
      + (0.1 * newSignalRate)
  ...
  return [...plan]
    .map((item) => {
      ...
      return {
        ...
        yield: scoreDiscoveryRunMetrics(metrics),
      };
    })
    .sort((left, right) => {
...
function withAllocationMetadata(item, { mode, start }) {
  return {
    ...item,
    input: {
      ...item.input,
      start,
      request_metadata: {
        ...item.input.request_metadata,
        start,
        discovery_page_start: start,
        discovery_allocation_mode: mode,
      },
    },
  };
}

export function getDiscoveryExploitationWindow(item) {
  if (item?.yield?.exploration) return { eligible: false, reason: "unmeasured" };

  const metrics = item?.historical_metrics ?? {};
  const limit = Number(metrics.requested_limit ?? item?.input?.limit ?? 50);
  const maxStart = Number(metrics.max_start ?? item?.input?.start ?? 1);
  const pageFetched = Number(metrics.max_start_fetched_count ?? 0);

  if (!Number.isInteger(limit) || limit < 1 || limit > 50) {
    return { eligible: false, reason: "invalid_limit" };
  }
  if (item.yield.score < DISCOVERY_MIN_EXPLOITATION_SCORE) {
    return { eligible: false, reason: "low_score" };
  }
  if (pageFetched < limit) {
    return { eligible: false, reason: "exhausted_page" };
  }

  const nextStart = maxStart + limit;
  if (nextStart + limit - 1 > 1000) {
    return { eligible: false, reason: "provider_window_exhausted" };
  }
...
function buildExploitationItem(item) {
  const window = getDiscoveryExploitationWindow(item);
  if (!window.eligible) return null;
  return withAllocationMetadata(item, { mode: "exploitation", start: window.next_start });
}

function buildExplorationItem(item) {
  return withAllocationMetadata(item, { mode: "exploration", start: item.input.start });
}
...
export function selectDiscoveryRequestBudget(plan, historicalMetrics = [], { maxRequests = 24 } = {}) {
  ...
  const exploration = roundRobinByDomain(
    ranked.filter((item) => item.yield.exploration).map(buildExplorationItem),
  );
  const exploitation = ranked
    .filter((item) => !item.yield.exploration)
    .map(buildExploitationItem)
    .filter(Boolean);
  ...
  const selectedKeys = new Set(selected.map((item) => `${item.query_key}:${item.input.start}`));

  for (const item of [...exploitation, ...exploration.slice(explorationSlots)]) {
    if (selected.length >= maxRequests) break;
    const allocationKey = `${item.query_key}:${item.input.start}`;
    if (selectedKeys.has(allocationKey)) continue;
    selected.push(item);
    selectedKeys.add(allocationKey);
  }
  return selected;
}