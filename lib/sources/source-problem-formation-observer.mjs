...
export const SOURCE_PROBLEM_FORMATION_OBSERVER_VERSION = "source-problem-formation-observer-v0.2";
export const SOURCE_PROBLEM_FORMATION_PROMPT_VERSION = "source-problem-formation-semantic-v0.1";
export const SOURCE_PROBLEM_FORMATION_RECOVERY_VERSION = "source-problem-formation-provider-recovery-v0.1";
export const SOURCE_PROBLEM_FORMATION_TIMEOUT_MS = 60_000;
export const SOURCE_PROBLEM_FORMATION_BASE_MAX_OUTPUT_TOKENS = 1_200;
export const SOURCE_PROBLEM_FORMATION_RECOVERY_MAX_OUTPUT_TOKENS = 2_400;

const SOURCE_PROBLEM_FORMATION_RECOVERY_INSTRUCTION =
  "Recovery attempt: the prior structured Formation response was incomplete. Return only the required structured fields, keep proposals concise, and avoid unnecessary reasoning or explanation.";
...
export function buildSourceProblemFormationJudgeRequest({
  title,
  fullText,
  sourcePlatform,
  model,
  ...
  if (providerRecovery) instructions.push(SOURCE_PROBLEM_FORMATION_RECOVERY_INSTRUCTION);

  return {
    promptVersion: SOURCE_PROBLEM_FORMATION_PROMPT_VERSION,
    recoveryVersion: SOURCE_PROBLEM_FORMATION_RECOVERY_VERSION,
    providerRecovery: Boolean(providerRecovery),
    body: {
      model,
      store: false,
      ...
      max_output_tokens: providerRecovery
...
export async function judgeSourceProblemFormationSemantics({
  ...
  return {
    ...
    provider_recovery_version: request.recoveryVersion,
    provider_recovery_applied: request.providerRecovery,
    provider: "openai",
...
export async function resolveSourceProblemFormationAudit(signal, {
  ...
  if (!judge) {
    ...
    judge = (input, control = {}) => judgeSourceProblemFormationSemantics({
      ...input,
      ...config,
      fetchImpl,
      providerRecovery: Boolean(control.recovery),
    });
  }

  const semanticAttemptLimit = Number.isInteger(maxSemanticAttempts) && maxSemanticAttempts > 0
    ? Math.min(maxSemanticAttempts, 2)
    : 2;
  let semantic = null;
  let lastError = null;
  let attemptCount = 0;
  ...
  while (attemptCount < semanticAttemptLimit) {
    ...
    try {
      ...
      if (recoveryAttempted) recoveryRecovered = true;
      break;
    } catch (error) {
      lastError = error;
      const retryableIncomplete = error?.code === "source_formation_provider_incomplete" && error?.retryable === true;
      if (!retryableIncomplete || recoveryAttempted || attemptCount >= semanticAttemptLimit) break;
...
function buildRecoveryMetadata({ attempted, recovered, attemptCount }) {
  return {
    version: SOURCE_PROBLEM_FORMATION_RECOVERY_VERSION,
    attempted: Boolean(attempted),
    recovered: Boolean(recovered),
    attempt_count: Number(attemptCount ?? 0),
    trigger_reason_code: attempted ? "source_formation_provider_incomplete" : null,
    base_max_output_tokens: SOURCE_PROBLEM_FORMATION_BASE_MAX_OUTPUT_TOKENS,
    recovery_max_output_tokens: attempted ? SOURCE_PROBLEM_FORMATION_RECOVERY_MAX_OUTPUT_TOKENS : null,
  };
}
