...
export async function listDiscoveryQueryMetrics(serviceClient, { limit = 1000 } = {}) {
  const { data, error } = await serviceClient
    .from("ar_source_ingestion_runs")
    .select("source_platform, query_text, request_metadata, fetched_count, inserted_count, duplicate_count, discovery_continue_count, discovery_reject_count, admission_candidate_count, admission_review_count, admission_reject_count, completed_at")
    .eq("status", "completed")
