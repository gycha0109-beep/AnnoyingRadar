...
export async function persistSourceSignals(serviceClient, {
  ...
  return { run, signals: persisted, observations, admission_summary: admissionSummary };
}
...
export async function listDiscoveryQueryMetrics(serviceClient, { limit = 1000 } = {}) {
  const { data, error } = await serviceClient
    .from("ar_source_ingestion_runs")
    .select("source_platform, query_text, request_metadata, requested_limit, fetched_count, inserted_count, duplicate_count, discovery_continue_count, discovery_reject_count, admission_candidate_count, admission_review_count, admission_reject_count, completed_at")
    .eq("status", "completed")
