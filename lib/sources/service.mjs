import { classifySourceAdmission, summarizeSourceAdmissions } from "./source-admission.mjs";
...
export async function getSourceAdmissionStats(serviceClient, { limit = 5000 } = {}) {
  const { data, error } = await serviceClient
    .from("ar_source_signals")
    .select("id, source_platform, canonical_url, raw_text, source_metadata")
    .eq("source_platform", "naver_blog")
    .order("last_seen_at", { ascending: false })
    .limit(limit);
  if (error) throw error;
  return summarizeSourceAdmissions(data ?? []);
}

export async function listSourceAdmissionQueue(serviceClient, { limit = 100 } = {}) {
  const fetchLimit = Math.max(limit * 10, 300);
  const { data, error } = await serviceClient
    .from("ar_source_signals")
