...
import { SOURCE_PROBLEM_FORMATION_PROMPT_VERSION } from "./source-problem-formation-observer.mjs";
...
function optionalText(value, name = "value", maxLength = null) {
  const normalized = String(value ?? "").trim();
...
export function validateFormationContextAgainstAdmission(sourceAdmissionOutcome, fullContext) {
  ...
  const declaredHash = String(fullContext.content_hash ?? "").trim();
  if (declaredHash && declaredHash !== currentHash) {
    throw new RangeError("Formation fetch content_hash does not match the exact fetched text");
  }
  ...
  if (!Number.isInteger(upstreamChars) || upstreamChars < 20) {
    throw new RangeError("Source Admission context char count is invalid");
  }
  ...
  return {
    ...
    context_extraction_scope: optionalText(fullContext.extraction_scope, "contextExtractionScope", 120),
  };
}
...
function recoveryMetadata(result) {
  ...
  return {
    recovery_version: optionalText(recovery.version, "recoveryVersion", 120),
    ...
    recovery_trigger_reason_code: optionalText(recovery.trigger_reason_code, "recoveryTriggerReasonCode", 160),
  };
}
...
export function buildSourceFormationAssessmentRow({
  ...
  const normalizedSourceSignalId = requiredText(sourceSignalId, "sourceSignalId", 80);
  if (String(sourceAdmissionOutcome?.source_signal_id ?? "").trim() !== normalizedSourceSignalId) {
    throw new RangeError("Formation Source Signal does not match Source Admission authority");
  }
  ...
  return {
    ...
    source_signal_id: normalizedSourceSignalId,
    ...
    ...fields,
    ...quote,
    problem_mechanism_proposal: optionalText(semantic?.problem_mechanism_proposal, "problemMechanismProposal", 240),
    incident_summary_proposal: optionalText(semantic?.incident_summary_proposal, "incidentSummaryProposal", 320),
    ...context,
    prompt_version: requiredText(semantic?.prompt_version ?? SOURCE_PROBLEM_FORMATION_PROMPT_VERSION, "promptVersion", 120),
