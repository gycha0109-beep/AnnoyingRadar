...
const GOLD_REVIEW_CANDIDATE_LIMIT = 500;
...
export async function listSourceSignalReviewQueue(serviceClient, { limit = 30 } = {}) {
  ...
  const [{ data: classifications, error: classificationError }, { data: gold, error: goldError }] = await Promise.all([
    serviceClient
  ...
  queue.sort((left, right) => Number(Boolean(left.gold_annotation)) - Number(Boolean(right.gold_annotation)));
