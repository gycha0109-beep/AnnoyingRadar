...
export async function recordCuratorIncidentDecision(serviceClient, {
  ...
  let packet;
  try {
    packet = await buildCuratorIncidentDecisionPacket(serviceClient, {
    ...
    if (error instanceof SourceIncidentDecisionPacketError) throw error;
    throw error;
  }
  ...
  return safeDecisionAuthority(data);
}
