...
function contextMetadata(fullContext) {
  ...
  return {
    ...
    context_scope: fullContext?.content_scope === "full_post" ? "full_post" : "full_post",
...
function recoveryMetadata(result) {
  ...
  if (recovery) {
    return {
      ...
      recovery_attempt_count: Number.isInteger(recovery.attempt_count) ? recovery.attempt_count : 0,
...
export async function persistSourceFullContextOutcome({
  ...
  const { data, error } = await service
    ...
    .insert(row)
    .select("id, batch_version, source_signal_id, status, decision, reason_codes, resolved_at, created_at")
    .single();
