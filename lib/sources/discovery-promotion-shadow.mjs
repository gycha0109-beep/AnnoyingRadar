...
} from "./review-promotion-calibration.mjs";

function clamp(value, min = 0, max = 1) {
  return Math.max(min, Math.min(max, value));
}

export function scoreDiscoveryRunMetricsWithPromotionShadow(metrics, { family } = {}) {
  const base = scoreDiscoveryRunMetrics(metrics);
  const calibration = getReviewPromotionCalibration({ family });
  const reviewRate = Number(base.rates?.review_rate ?? 0);

  if (base.exploration || reviewRate <= 0) {
    return {
      version: REVIEW_PROMOTION_SHADOW_VERSION,
      family: family ?? null,
...
export function buildPromotionShadowRanking(plan, historicalMetrics = []) {
  const byKey = new Map(historicalMetrics.map((row) => [row.query_key, row]));
  return [...plan]
    .map((item) => {
      const metrics = byKey.get(item.query_key) ?? { completed_runs: 0 };
      const shadow = scoreDiscoveryRunMetricsWithPromotionShadow(metrics, { family: item.family });
      const baseItem = {
        ...item,
      ...
      return {
        ...
        review_count: Number(metrics.new_admission_review_count ?? metrics.admission_review_count ?? 0),
...
export function summarizePromotionShadow(ranking) {
  const measured = ranking.filter((row) => row.completed_runs > 0);
  const crossings = measured.filter(
