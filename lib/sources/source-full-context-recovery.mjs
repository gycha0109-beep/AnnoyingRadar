...
function normalizeEligibleReasonCodes(reasonCodes = SOURCE_FULL_CONTEXT_RECOVERY_REASON_CODES) {
  if (!Array.isArray(reasonCodes) || reasonCodes.length === 0) {
    throw new TypeError("eligibleReasonCodes must be a non-empty array");
  }
  const normalized = [...new Set(reasonCodes.map((code) => String(code ?? "").trim()).filter(Boolean))];
  if (normalized.length === 0) throw new TypeError("eligibleReasonCodes must contain at least one reason code");
  for (const code of normalized) {
    if (!RECOVERY_REASON_SET.has(code)) {
      throw new RangeError(`Unsupported recovery reason code: ${code}`);
    }
  }
...
export function isSourceFullContextRecoveryEligible(error, {
  eligibleReasonCodes = SOURCE_FULL_CONTEXT_RECOVERY_REASON_CODES,
} = {}) {
  const eligible = normalizeEligibleReasonCodes(eligibleReasonCodes);
  return eligible.has(String(error?.code ?? ""));
}
...
export async function runSourceFullContextJudgeWithRecovery(judge, input, {
  ...
  try {
    ...
    if (!isSourceFullContextRecoveryEligible(error, { eligibleReasonCodes: eligible })) {
      return {
        semantic: null,
        error,
