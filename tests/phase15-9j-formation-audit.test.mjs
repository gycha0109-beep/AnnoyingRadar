...
import { PHASE15_9I_CANDIDATE_AUTHORITY } from "../lib/sources/phase15-9i-confirmed-fn-outcome-persistence.mjs";
...
test("15.9J durable authority validation fails closed on semantic or persisted-context drift", () => {
  const semanticDrift = durableRow(4);
  semanticDrift.content_kind = "informational";
  assert.throws(() => validatePhase15_9JOutcomeAuthority([semanticDrift, durableRow(9), durableRow(16)]), /content_kind drifted/);
  const contextDrift = durableRow(9);
  contextDrift.context_char_count += 1;
  assert.throws(() => validatePhase15_9JOutcomeAuthority([durableRow(4), contextDrift, durableRow(16)]), /context_char_count drifted/);
});

test("15.9J current-context drift is isolated without relaxing frozen authority", () => {
  const authority = PHASE15_9I_CANDIDATE_AUTHORITY[4];
  const target = { baseline_ordinal: 4, h_authority: authority };
  const context = {
    status: "resolved",
    content_scope: "full_post",
    content_text: "x".repeat(authority.context_chars),
  ...
  const integrity = inspectPhase15_9JContextIntegrity(context, { ...context }, target, {
    compareFetches: () => ({ stable: true }),
  });
  assert.equal(integrity.ok, false);
  assert.equal(integrity.stable_pair, true);
  assert.ok(integrity.failures.includes("first_content_hash_drift"));
  assert.ok(integrity.failures.includes("second_content_hash_drift"));
  assert.equal(integrity.observed_first.content_hash, "new-current-hash");
  assert.equal("content_text" in integrity.observed_first, false);

  const item = buildPhase15_9JContextDriftItem({ target, integrity });
  assert.equal(item.audit_status, "context_drift");
  assert.equal(item.formation_state, null);
  assert.equal(item.recovery.attempt_count, 0);
});

test("15.9J artifact hashes the exact Formation evidence quote instead of emitting it", () => {
  const target = { baseline_ordinal: 4, h_authority: PHASE15_9I_CANDIDATE_AUTHORITY[4] };
  ...
  const item = buildPhase15_9JArtifactItem({
    ...
    formationResult: {
      ...
      semantic: {
        problem_claim: "yes", experience_actor: "self", friction_specificity: "concrete", pain_centrality: "central",
        content_kind: "organic", source_origin: "original", friction_responsibility: "external_service_or_product",
        evidence_quote: "exact friction quote", problem_mechanism_proposal: "repeatable mechanism", incident_summary_proposal: "one episode",
      },
      recovery: { attempted: false, recovered: false, attempt_count: 1, trigger_reason_code: null },
    },
  });
  assert.equal(item.audit_status, "formation_evaluated");
...
test("15.9J runner skips model evaluation for drifted context and remains DB read-only", async () => {
