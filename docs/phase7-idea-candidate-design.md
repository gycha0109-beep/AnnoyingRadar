...
The additional `completed` requirement is deliberate. Current v0.1 DB RPCs allow Candidate status changes only while `reviewing_candidates`; generating ideas before completion would allow the source Candidate to return to draft/discarded after Idea rows already reference it. Requiring `completed` makes the Problem Card source stable without adding snapshot semantics or destructive cascades to user-authored Idea data.
...
Provider output treats `title`, `one_liner`, `problem_statement`, `core_value`, `first_build_scope`, `implementation_difficulty`, `monetization_hint`, and `first_screen_idea` as required structured fields. `target_user` may be null when the source does not support a specific target user; the model must not invent one merely to fill the schema. `excluded_scope` may be null when a meaningful exclusion cannot be derived from the proposed first-build scope.
...
excluded_scope text null
implementation_difficulty text not null
monetization_hint text not null
first_screen_idea text not null
...
```text
candidate / researching / build_soon / paused → editable
discarded / archived → read-only until moved back to an active state
```

Status is organizational; moving to Researching or Build Soon does not freeze the Idea content.
...
- batch + Idea Candidate + initial status event insert is atomic.
...
`PATCH /api/idea-candidates/{ideaId}` edits content/memo only while the current status is `candidate`, `researching`, `build_soon`, or `paused`; it does not change `status`.
...
A confirmed Candidate can still return to draft while `reviewing_candidates`, and current DB RPCs explicitly permit those transitions only in that state. That would violate the Idea source invariant. Phase 7 requires a completed source analysis. Accepted correction.
...
### DR-09 — Do not invent target users to satisfy the output schema

Current Problem Card target-user data is nullable. Forcing AI to return a non-null target user would turn missing source evidence into an invented claim. `target_user` therefore remains nullable in the structured output and persistence contract.

### DR-10 — Status must not freeze active research edits

Researching/Build Soon are organizational states, not immutable approvals. Active Idea content remains editable; only Discarded/Archived are read-only until restored.
...
- nullable target-user behavior avoids unsupported AI invention.
