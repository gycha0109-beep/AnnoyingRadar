...
6. Do not use snippet text alone to promote a result.
...
- `reject`: title establishes informational/guide or positive-review intent.

## Examples locked by tests

- `카카오톡 로그인 오류·계정 도용·결제 문제, 상황별 해결 경로 정리 직접 해봤어요` → reject.
- `so what? we hot we young` with one incidental `환불안됨` phrase in the snippet → review, never candidate from snippet alone.
...
The existing 120-sample blind human evaluation remains untouched. Phase 15.5E does not read classifier outputs into the blind labeling flow and does not modify the existing DB blind guards.
