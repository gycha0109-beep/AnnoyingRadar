...
const RUNNER_PATH = path.join(PROJECT_ROOT, "scripts", "run-problem-alternatives-live-e2e.mjs");
...
console.log(`Problem Alternatives Live E2E 전용 서버 주소: ${baseUrl}`);
...
function assertProjectRoot() {
  for (const requiredPath of ["package.json", path.join("scripts", "run-problem-alternatives-live-e2e.mjs")]) {
...
function assertManagedEnvironment() {
  ...
  if (missing.length > 0) {
    throw new Error(
      `Problem Alternatives Live E2E에 필요한 환경변수가 없습니다: ${missing.join(", ")}. `
...
async function allocateLoopbackBaseUrl() {
  ...
  if (!address || typeof address === "string") {
    server.close();
    throw new Error("Problem Alternatives Live E2E용 loopback port를 할당하지 못했습니다.");
  }
...
async function runInnerRunner() {
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", (code, signal) => {
      if (signal) {
        reject(new Error(`Problem Alternatives Live E2E runner가 signal=${signal}로 종료되었습니다.`));
        return;
      }
      resolve(code ?? 1);
    });
