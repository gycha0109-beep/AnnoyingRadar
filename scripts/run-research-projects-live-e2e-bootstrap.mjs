...
const RUNNER_PATH = path.join(PROJECT_ROOT, "scripts", "run-research-projects-live-e2e.mjs");
...
console.log(`Research Project Live E2E 전용 서버 주소: ${baseUrl}`);
...
try {
  if (!(await serverAvailable(baseUrl))) {
    ...
    if (!isLoopback(url.hostname)) {
      throw new Error(`E2E_BASE_URL에 연결할 수 없습니다: ${baseUrl}`);
    }
...
function assertProjectRoot() {
  for (const requiredPath of [
    "package.json",
    path.join("scripts", "run-research-projects-live-e2e.mjs"),
  ]) {
...
function assertManagedEnvironment() {
  ...
  if (missing.length > 0) {
    throw new Error(
      `Research Project Live E2E에 필요한 환경변수가 없습니다: ${missing.join(", ")}. `
...
async function allocateLoopbackBaseUrl() {
  ...
  if (!address || typeof address === "string") {
    server.close();
    throw new Error("Research Project Live E2E용 loopback port를 할당하지 못했습니다.");
  }
...
async function startApplicationServer(url) {
  ...
  const earlyExit = new Promise((_, reject) => {
    child.once("error", (error) => reject(
      new Error(`개발 서버를 시작하지 못했습니다 (${command.label}): ${error.message}`),
    ));
...
async function runInnerRunner() {
  return new Promise((resolve, reject) => {
    ...
    child.once("exit", (code, signal) => {
      if (signal) {
        reject(new Error(`Research Project Live E2E runner가 signal=${signal}로 종료되었습니다.`));
        return;
      }
      resolve(code ?? 1);
    });
