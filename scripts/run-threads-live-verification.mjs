...
function assertSignalContract(signal, scenarioName) {
  ...
  if (signal.canonical_url !== null) {
    assert.match(signal.canonical_url, /^https?:\/\//, `${scenarioName}: canonical_url`);
  }
  if (signal.published_at !== null) {
    assert.ok(!Number.isNaN(Date.parse(signal.published_at)), `${scenarioName}: published_at`);
  }
}
...
async function main() {
  if (!process.env.THREADS_ACCESS_TOKEN) {
    throw new Error("THREADS_ACCESS_TOKEN is missing; load .env.local before running this verification");
  }
  ...
  const scenarios = [
    {
      name: "complaint_recent",
      input: { q: "배달 불편", search_type: "RECENT", search_mode: "KEYWORD", limit: 10 },
      persist: true,
    },
    {
      name: "neutral_recent",
      input: { q: "배달", search_type: "RECENT", search_mode: "KEYWORD", limit: 10 },
      persist: true,
    },
    {
      name: "noise_top",
      input: { q: "추천", search_type: "TOP", search_mode: "KEYWORD", limit: 10 },
      persist: true,
    },
    {
      name: "top_compare",
      input: { q: "배달", search_type: "TOP", search_mode: "KEYWORD", limit: 10 },
      persist: false,
    },
    {
      name: "tag_recent",
      input: { q: "배달", search_type: "RECENT", search_mode: "TAG", limit: 10 },
      persist: false,
    },
    {
      name: "window_recent",
    ...
    {
      name: "limit_one",
      input: { q: "앱", search_type: "RECENT", search_mode: "KEYWORD", limit: 1 },
      persist: false,
    },
  ];
  ...
  for (const scenario of scenarios) {
    ...
    scenarioSummary.push({
      scenario: scenario.name,
      q: result.input.q,
      search_type: result.input.search_type,
  ...
  assert.throws(
    () => normalizeThreadsSearchInput({ q: "", search_type: "RECENT", search_mode: "KEYWORD", limit: 10 }),
    /q must contain/,
  );
  assert.throws(
    () => normalizeThreadsSearchInput({ q: "배달", search_type: "RECENT", search_mode: "KEYWORD", limit: 51 }),
    /limit must be/,
  );
  assert.throws(
    () => normalizeThreadsSearchInput({
      ...
      search_type: "RECENT",
      search_mode: "KEYWORD",
      since: until.toISOString(),
      until: since.toISOString(),
    }),
    /since must be earlier/,
  );
  await assert.rejects(
    () => searchThreadsPosts({ q: "배달", search_type: "RECENT", search_mode: "KEYWORD", limit: 1 }, { accessToken: "" }),
    (error) => error instanceof ThreadsAdapterError && error.code === "threads_not_configured",
  );
  ...
  if (!adapterOnly) {
    ...
    for (const scenario of scenarios.filter((item) => item.persist)) {
      persistedRuns.push(
        await persistScenario(client, curatorUserId, scenario, results.get(scenario.name)),
      );
    }

    const repeatScenario = scenarios.find((item) => item.name === "complaint_recent");
    const repeatResult = results.get("complaint_recent");
    const repeat = await persistScenario(client, curatorUserId, {
      ...repeatScenario,
      name: "complaint_recent_repeat",
    }, repeatResult);
    persistedRuns.push(repeat);

    const expectedRepeatDuplicates = uniqueCount(repeatResult.signals);
    assert.equal(
      repeat.duplicate_count,
      expectedRepeatDuplicates,
      "Repeated persistence must dedupe every previously persisted Source Signal",
    );
    assert.equal(
      repeat.observation_count,
      expectedRepeatDuplicates,
      "Repeated persistence must create a new Observation for every deduped Source Signal",
    );
  ...
  console.log(JSON.stringify({
    ...
    comparisons: {
      neutral_recent_vs_top_overlap: topRecentOverlap,
    },
    local_contract_checks: {
      empty_query_rejected: true,
      limit_51_rejected: true,
      reversed_window_rejected: true,
      missing_token_rejected_without_network_call: true,
    },
    persistence,
  }, null, 2));
}
